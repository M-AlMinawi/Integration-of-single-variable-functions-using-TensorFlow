...
def integrate(func, lower, upper):
    return (func(lower) + 4*func((lower+upper)/2) +  func(upper)) / 6 * (upper - lower), func(lower),func(upper)


initial_points = tf.linspace(tf.cast(0,dtype=tf.float64), tf.cast(1,dtype=tf.float64), num=800)

func = lambda x: 1/(1 + 2500* x**2)

@tf.function(autograph=False)
def body(integral,increase,tol,lower,upper):
    integrals = integrate(func,lower,upper)
    gradients = tf.abs(func(upper)-func(lower)/tf.abs(upper-lower))
    too_big = tf.greater(gradients,tf.reduce_mean(gradients))
    points  = tf.where(too_big)[:, 0]
    increase = tf.reduce_sum(tf.boolean_mask(integrals, mask=tf.logical_not(too_big)), axis=0)
    integral += increase
...
def diff_body(integral,increase,tol,lower,upper,plot_points,rejected_points,iterations):
    ...
    abs_diff_bound = tf.cond(tf.math.greater(tol,tf.cast(1e-10,dtype=tf.float64)),lambda :tf.cond(tf.math.greater(tol,1e-6),
                                                                       lambda: tf.abs((tf.cast(16, dtype=tf.float64) * tol**(1/4))),
                                                                       lambda : tf.abs(1250 * ((tol ** (1 / 2)))))
                             ,lambda :tf.abs(5000* ((tol ** (2 / 3)))))
...
@tf.function(autograph=False)
def adaptive_simpson(l,u,tol):
    initial_points = tf.linspace(tf.cast(l,dtype=tf.float64), tf.cast(u,dtype=tf.float64), num=100)
    return tf.while_loop(cond=cond, body=body, loop_vars=[tf.cast(0.,dtype=tf.float64),
...
@tf.function(autograph=False)
def adaptive_simpson_diff(l,u,tol,iter):
    initial_points = tf.linspace(tf.cast(l,dtype=tf.float64), tf.cast(u,dtype=tf.float64), num=75)
